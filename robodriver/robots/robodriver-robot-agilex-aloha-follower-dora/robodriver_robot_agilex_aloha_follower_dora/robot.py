...
from .node import AgilexAlohaFollowerDoraRobotNode


logger = logging_mp.get_logger(__name__)


class AgilexAlohaFollowerDoraRobot(Robot):
    config_class = AgilexAlohaFollowerDoraRobotConfig
    name = "agilex_aloha_follower_dora"

    def __init__(self, config: AgilexAlohaFollowerDoraRobotConfig):
        ...
        self.status = AgilexAlohaFollowerDoraRobotStatus()
        self.robot_dora_node = AgilexAlohaFollowerDoraRobotNode()
    ...
    def connect(self):
        ...
        conditions = [
            ...
            (
                ...
                lambda: [] if len(self.robot_dora_node.recv_follower_joint_right) > 0 else ["recv_follower_joint_right"],
                "等待右从臂关节角度超时",
            ),
            (
                lambda: len(self.robot_dora_node.recv_follower_joint_left) > 0,
                lambda: [] if len(self.robot_dora_node.recv_follower_joint_left) > 0 else ["recv_follower_joint_left"],
                "等待左从臂关节角度超时",
            ),
        ]
                ...
                for i in range(len(completed)):
                    if not completed[i]:
                        ...
                        if i == 0:  # 摄像头条件
                            ...
                            # 对于关节角度条件，missing要么是空列表，要么是["recv_follower_joint_right"]或["recv_follower_joint_left"]
                            if i == 1:  # 右从臂关节角度
        ...
        # 右从臂关节角度状态
        if conditions[1][0]():
            # 检查recv_follower_joint_right中实际接收到的数据
            success_messages.append(f"右从臂关节角度: 已接收 ({len(self.robot_dora_node.recv_follower_joint_right)}个数据点)")

        # 左从臂关节角度状态
        if conditions[2][0]():
            # 检查recv_follower_joint_left中实际接收到的数据
            success_messages.append(f"左从臂关节角度: 已接收 ({len(self.robot_dora_node.recv_follower_joint_left)}个数据点)")
    ...
    def update_status(self) -> str:
        ...
        self.status.specifications.arm.information[0].is_connect = (
            True if self.robot_dora_node.recv_follower_joint_right_status > 0 else False
        )
        self.status.specifications.arm.information[1].is_connect = (
            True if self.robot_dora_node.recv_follower_joint_left_status > 0 else False
        )
