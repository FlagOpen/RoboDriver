...
if __name__ == "__main__":
    print("--- Init Robot ---")
    ik = Arm_IK("urdf/piper.urdf")
    
    # 1. Zero Position FK
    q_zero = np.zeros(ik.model.nq)
    print(f"q_zero: {q_zero}")
    pose_start = ik.get_fk(q_zero)
    print(f"[FK] Zero Pos: xyz={pose_start[:3].round(4)}, rpy={pose_start[3:].round(4)}")

    # 2. IK Solve (Move X to 1.0)
    target = pose_start.copy()
    target[0] = 1
    print(f"[IK] Target:   x={target[0]:.4f}, y={target[1]:.4f}, z={target[2]:.4f}")
    
    q_sol = ik.get_ik_solution(*target)
    
    if q_sol is not None:
        print(f"[OK] Solution: {q_sol.round(4)}")
        # 3. Verify
        pose_end = ik.get_fk(q_sol)
        error = np.linalg.norm(pose_end[:3] - target[:3])
        print(f"[FK] Reached:  xyz={pose_end[:3].round(4)}, rpy={pose_end[3:].round(4)}")
        print(f"[Err] Pos Diff: {error:.6f} m")
    else:
